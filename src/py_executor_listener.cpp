...
namespace ratio::python
{
    ...
    void py_executor_listener::start(const std::unordered_set<ratio::atom *> &atoms)
    {
        pybind11::set atms;
        for (const auto &atm : atoms)
            atms.add(py_exec.attr("solver").attr("atoms")[std::to_string(get_id(*atm)).c_str()]);
    ...
    void py_executor_listener::ending(const std::unordered_set<ratio::atom *> &atoms)
    {
        pybind11::set atms;
        for (const auto &atm : atoms)
            atms.add(py_exec.attr("solver").attr("atoms")[std::to_string(get_id(*atm)).c_str()]);
    ...
    void py_executor_listener::end(const std::unordered_set<ratio::atom *> &atoms)
    {
        pybind11::set atms;
        for (const auto &atm : atoms)
            atms.add(py_exec.attr("solver").attr("atoms")[std::to_string(get_id(*atm)).c_str()]);
